...
print("--- PASSO 3 e 4: Leitura dos dados ---")
# Lendo os dados com separador ponto e vírgula (;)
# Tentar vários nomes de arquivo no mesmo diretório do script; se não existir, usar o sample_data interno
script_dir = os.path.dirname(os.path.abspath(__file__))
candidate_files = [
    os.path.join(script_dir, 'sample_data.csv'),
    os.path.join(script_dir, 'data_sample.csv'),
    os.path.join(script_dir, 'sample-data.csv'),
    os.path.join(script_dir, 'dados.csv')
]
...
for fp in candidate_files:
    if os.path.exists(fp):
        try:
            dados_originais = pd.read_csv(fp, sep=';', na_values='NaN')
            print(f"✓ Dados lidos a partir do arquivo: {fp}")
            break
        except Exception as e:
            print(f"Erro ao ler {fp}: {e}")

if dados_originais is None:
    print("Nenhum arquivo de dados encontrado nos caminhos previstos. Utilizando dados embutidos de exemplo.")
